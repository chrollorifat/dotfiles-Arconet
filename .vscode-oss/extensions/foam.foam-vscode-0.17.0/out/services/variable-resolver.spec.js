...
describe('substituteFoamVariables', () => {
    ...
    `;
        ...
        const resolver = new variable_resolver_1.Resolver(givenValues, new Date());
...
describe('resolveFoamVariables', () => {
    ...
    test('Resolves FOAM_DATE_* properties with current day by default', async () => {
        ...
        const expected = new Map();
        expected.set('FOAM_DATE_YEAR', new Date().toLocaleString('default', { year: 'numeric' }));
        expected.set('FOAM_DATE_MONTH_NAME', new Date().toLocaleString('default', { month: 'long' }));
        expected.set('FOAM_DATE_DATE', new Date().toLocaleString('default', { day: '2-digit' }));
        const givenValues = new Map();
        const resolver = new variable_resolver_1.Resolver(givenValues, new Date());
        expect(await resolver.resolveAll(variables)).toEqual(expect.objectContaining(expected));
    });
    test('Resolves FOAM_DATE_* properties with given date', async () => {
        ...
        expected.set('FOAM_DATE_SECONDS_UNIX', (targetDate.getTime() / 1000).toString());
        const givenValues = new Map();
        const resolver = new variable_resolver_1.Resolver(givenValues, targetDate);
        expect(await resolver.resolveAll(variables)).toEqual(expected);
    });
});
describe('resolveFoamTemplateVariables', () => {
    ...
    `;
        ...
        const resolver = new variable_resolver_1.Resolver(new Map(), new Date(), new Set(['FOAM_TITLE']));
        expect(await resolver.resolveText(input)).toEqual(expected);
    });
    test('Appends FOAM_SELECTED_TEXT with a newline to the template if there is selected text but FOAM_SELECTED_TEXT is not referenced and the template ends in a newline', async () => {
        ...
        const resolver = new variable_resolver_1.Resolver(givenValues, new Date(), new Set(['FOAM_TITLE', 'FOAM_SELECTED_TEXT']));
        expect(await resolver.resolveText(input)).toEqual(expected);
    });
...
//# sourceMappingURL=variable-resolver.spec.js.map