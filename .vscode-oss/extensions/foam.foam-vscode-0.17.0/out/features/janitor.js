...
async function runJanitor(foam) {
    const notes = foam.workspace
        .list()
        .filter(r => uri_1.URI.isMarkdownFile(r.uri));
    ...
    const dirtyNotes = notes.filter(note => dirtyEditorsFileName.includes(uri_1.URI.toFsPath(note.uri)));
    const nonDirtyNotes = notes.filter(note => !dirtyEditorsFileName.includes(uri_1.URI.toFsPath(note.uri)));
    ...
    const fileWritePromises = nonDirtyNotes.map(note => {
        let heading = janitor_1.generateHeading(note);
        if (heading) {
        ...
        return fs.promises.writeFile(uri_1.URI.toFsPath(note.uri), text);
    });
    ...
    for (const doc of dirtyTextDocuments) {
        ...
        const note = dirtyNotes.find(n => uri_1.URI.toFsPath(n.uri) === editor.document.uri.fsPath);
        // Get edits
