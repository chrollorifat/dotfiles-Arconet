...
function updateDocumentInNoteGraph(foam, document) {
    foam.workspace.set(foam.services.parser.parse(vsc_utils_1.fromVsCodeUri(document.uri), document.getText()));
}
...
function generateReferenceList(foam, doc) {
    ...
    const note = foam.get(vsc_utils_1.fromVsCodeUri(doc.uri));
    ...
    const references = lodash_1.uniq(markdown_provider_1.createMarkdownReferences(foam, note.uri, wikilinkSetting === settings_1.LinkReferenceDefinitionsSetting.withExtensions).map(markdown_provider_1.stringifyMarkdownLinkReferenceDefinition));
    if (references.length) {
        return [
            janitor_1.LINK_REFERENCE_DEFINITION_HEADER,
            ...references,
            janitor_1.LINK_REFERENCE_DEFINITION_FOOTER,
        ];
    }
    return [];
}
...
function detectReferenceListRange(doc) {
    ...
    const headerIndex = fullText.indexOf(janitor_1.LINK_REFERENCE_DEFINITION_HEADER);
    const footerIndex = fullText.lastIndexOf(janitor_1.LINK_REFERENCE_DEFINITION_FOOTER);
    if (headerIndex < 0) {
        return null;
    }
    ...
    return new vscode_1.Range(new vscode_1.Position(headerLine, 0), new vscode_1.Position(footerLine, janitor_1.LINK_REFERENCE_DEFINITION_FOOTER.length));
}
