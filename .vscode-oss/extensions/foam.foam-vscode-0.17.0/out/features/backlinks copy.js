    ...
    getChildren(item) {
        ...
        if (item) {
            ...
            const backlinkRefs = Promise.all(resource.links
                .filter(link => uri_1.URI.isEqual(this.workspace.resolveLink(resource, link), uri))
                .map(async (link) => {
                var _a;
        ...
        const backlinksByResourcePath = lodash_1.groupBy(this.graph.getConnections(uri).filter(c => uri_1.URI.isEqual(c.target, uri)), b => b.source.path);
...
//# sourceMappingURL=backlinks copy.js.map