...
describe('Backlinks panel', () => {
    beforeAll(async () => {
        ...
        await test_utils_vscode_1.createNote(noteA);
        await test_utils_vscode_1.createNote(noteB);
        await test_utils_vscode_1.createNote(noteC);
    });
    afterAll(async () => {
        graph.dispose();
        ws.dispose();
    ...
    const rootUri = test_utils_vscode_1.getUriInWorkspace('just-a-ref.md');
    ...
    it.skip('does not target excluded files', async () => {
        provider.target = uri_1.URI.file('/excluded-file.txt');
